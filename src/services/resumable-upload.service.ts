...
export class ResumableUploadService {
  ...
  private handleProgress(fileName: string, onProgress?: (bytesUploaded: number, bytesTotal: number) => void) {
    return (bytesUploaded: number, bytesTotal: number) => {
      const percentage = ((bytesUploaded / bytesTotal) * 100).toFixed(2);
      console.log(`Upload progress for ${fileName}: ${bytesUploaded}/${bytesTotal} bytes (${percentage}%)`);
  ...
  async uploadFile(options: ResumableUploadOptions): Promise<string> {
    ...
    return new Promise((resolve, reject) => {
      const upload = new tus.Upload(file, {
        ...
        metadata: this.createMetadata(bucketName, fileName, contentType, metadata),
